                ...
                sh """#!/bin/bash
                  ...
                  if [[ \${indexdRecord} -le 0 ]]; then
                    curl -X POST 'https://${TARGET_ENVIRONMENT}.planx-pla.net/index/index' -H "${QA_DCP_CREDS_JSON}" -H "Content-Type: application/json" -d \
                      '{"authz":["/gen3/programs/QA/projects/test"],"file_name":"qa-test.txt","form":"object","hashes":{"md5":"404e8919021a03285697647487f528ef"},"size":2681688756,"urls":["gs://dcf-integration-qa/qa-test.txt", "s3://cdis-presigned-url-test/testdata"]}' #pragma: allowlist secret
                  else
